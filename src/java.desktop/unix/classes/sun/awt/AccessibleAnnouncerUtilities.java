...
private static String getSpeechServerInfo(Object conf) {
    ...
    String profielName = getActiveProfile(conf);
    if (profielName == null) return null;

    return JSONParser.getStringValue(conf, PROFILES_KEY, profielName, SPEECH_SERVER_INFO_KEY, String.valueOf(0));
}
...
private static double getGain(Object conf) {
    ...
    String profielName = getActiveProfile(conf);
    if (profielName == null) return -1;

    Double val = JSONParser.getDoubleValue(conf, PROFILES_KEY, profielName, VOICES_KEY, DEFAULT_KEY, GAIN_KEY);
...
private static String getVariant(Object conf) {
    ...
    return JSONParser.getStringValue(conf, PROFILES_KEY, profielName, VOICES_KEY, DEFAULT_KEY, FAMILY_KEY, VARIANT_KEY);
}
...
private static String getDialect(Object conf) {
    ...
    return JSONParser.getStringValue(conf, PROFILES_KEY, profielName, VOICES_KEY, DEFAULT_KEY, FAMILY_KEY, DIALECT_KEY);
}
...
private static String getLang(Object conf) {
    ...
    return JSONParser.getStringValue(conf, PROFILES_KEY, profielName, VOICES_KEY, DEFAULT_KEY, FAMILY_KEY, LANG_KEY);
}
...
private static String getName(Object conf) {
    ...
    return JSONParser.getStringValue(conf, PROFILES_KEY, profielName, VOICES_KEY, DEFAULT_KEY, FAMILY_KEY, NAME_KEY);
}
...
private static double getAveragePitch(Object conf) {
    ...
    Double val = JSONParser.getDoubleValue(conf, PROFILES_KEY, profielName, VOICES_KEY, DEFAULT_KEY, AVERAGE_PITCH_KEY);
    if (val != null) return val;
    return -1;
}
...
private static double getRate(Object conf) {
    ...
    Double val = JSONParser.getDoubleValue(conf, PROFILES_KEY, profielName, VOICES_KEY, DEFAULT_KEY, RATE_KEY);
    if (val != null) return val;
    return -1;
}
...
private static boolean getEstablished(Object conf) {
    ...
    String profielName = getActiveProfile(conf);
    if (profielName == null) return false;

    Boolean val = JSONParser.getBooleanValue(conf, PROFILES_KEY, profielName, VOICES_KEY, DEFAULT_KEY, ESTABLISHED_KEY);
    if (val != null) return val;
    return false;
}
        ...
        OBJECT {
            Object parse(CharacterIterator it) throws ParseException {
                ...
                while (it.current() != '}' && it.current() != CharacterIterator.DONE) {
                    ...
                    Token.skipWhitespace(it);
                    if (name == null) error(it, "error parsing member name in object");
                    Token.skipOver(it, ':');
